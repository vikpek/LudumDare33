...
public class EnemyController : ExtendedMonoBehaviour
{
    ...
    void SelfDesctructionProcedure()
    {
        ...
        if (gameObject.name == "EnemyBig(Clone)")
        {
            ...
            GameObject.FindGameObjectWithTag(Tags.GAME_CONTROLLER).GetComponent<GameController>().IncreasePoints(1000);
        }
        if (gameObject.name == "EnemyQuick(Clone)")
        {
            GameObject.FindGameObjectWithTag(Tags.GAME_CONTROLLER).GetComponent<GameController>().IncreaseCash(100);
            GameObject.FindGameObjectWithTag(Tags.GAME_CONTROLLER).GetComponent<GameController>().IncreasePoints(1000);
        }
        if (gameObject.name == "Enemy(Clone)")
        {
            GameObject.FindGameObjectWithTag(Tags.GAME_CONTROLLER).GetComponent<GameController>().IncreaseCash(50);
            GameObject.FindGameObjectWithTag(Tags.GAME_CONTROLLER).GetComponent<GameController>().IncreasePoints(50);
        }
    }
